...
///
/// Provides deterministic, set-based input handling for the rust-bridge tool.
/// All path inputs are:
...
mod tests {
    ...
    fn test_normalize_deduplicates() {
        ...
        let file1 = temp.path().join("test.coreir");
    ...
    fn test_directory_expansion() {
        ...
        fs::write(dir.join("a.coreir"), "a").unwrap();
        fs::write(dir.join("b.coreir"), "b").unwrap();
        
        let subdir = dir.join("sub");
        fs::create_dir(&subdir).unwrap();
        fs::write(subdir.join("c.coreir"), "c").unwrap();

        let inputs = vec![dir.to_path_buf()];
        let result = normalize_path_inputs(&inputs, Some("coreir")).unwrap();

        // Should find all 3 .coreir files
        assert_eq!(result.len(), 3);
    }

    #[test]
    fn test_derive_name() {
        let path = PathBuf::from("path/to/myfile.coreir");
