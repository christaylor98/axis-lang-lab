...
A registry file consists of:

1. **A mandatory registry header**
2. **Zero or more function records**

The file is line-oriented and declarative.

---

### 3.1 Registry Header (Mandatory)

Each `.axreg` file MUST begin with exactly one registry header:
...
1. The header MUST be the first non-comment, non-whitespace line.
2. `<name>` is an opaque identifier for the registry.
3. `<version>` is an opaque version token.
4. The header declares **registry identity only**.
5. The header has **no semantic effect** on function behavior.
6. Exactly one header per file is permitted.

A file missing a valid registry header MUST be rejected.

---

### 3.2 Function Record Structure
...
Registry **identity and versioning** are declared by the header but interpreted
only by the host environment.
